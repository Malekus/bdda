...
CREATE OR REPLACE FUNCTION totalNbOccu(tableData IN VARCHAR, colData IN VARCHAR, maTable IN VARCHAR, maColonne IN VARCHAR)
RETURN NUMBER
AS
  ...
  res NUMBER := 0;
BEGIN
  OPEN vCursor FOR ('SELECT ' || UPPER(maColonne) || ' FROM ' || UPPER(maTable));
    LOOP
      FETCH vCursor INTO maVariable;
          res := res + nbOccColonne(tableData, colData, maVariable);
...
CREATE OR REPLACE PROCEDURE correctionAvcDD(maTable IN VARCHAR)
AS
BEGIN
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW V_TEMP_COMP AS SELECT  X.COLUMN_NAME, Y.TABLE_NAME AS TAB_DICT, Y.COLUMN_NAME AS COL_DICT FROM (SELECT column_name FROM user_tab_cols WHERE TABLE_NAME = ''' || 
  UPPER(maTable) || ''') X, (SELECT TABLE_NAME, COLUMN_NAME FROM DD_COL_NAME) Y';
  FOR ligne in (SELECT * FROM V_TEMP_COMP) LOOP
    
  END LOOP;
END;
/

CREATE OR REPLACE VIEW V_TEMP_COMP AS SELECT  X.COLUMN_NAME, Y.TABLE_NAME AS TAB_DICT, Y.COLUMN_NAME AS COL_DICT  FROM (SELECT column_name FROM user_tab_cols WHERE TABLE_NAME = 'CLIENTS') X, 
(SELECT TABLE_NAME, COLUMN_NAME FROM DD_COL_NAME) Y;

-- , totalNbOccu(Y.TABLE_NAME, Y.COLUMN_NAME, 'clients', X.COLUMN_NAME) as nb

select totalNbOccu('DATA_DICTIONARY_COUNTRY', 'COUNTRY_ENGLISH', 'clients', 'PAYSCLI') from dual;

select * from V_TEMP_COMP;



select * from V_ALL_CLIENTS;

CREATE OR REPLACE PROCEDURE allCustomers
AS
BEGIN
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW V_ALL_CLIENTS as SELECT TABLE_NAME, COLUMN_NAME FROM user_tab_cols WHERE TABLE_NAME LIKE ''CLIENT%'' group by TABLE_NAME, COLUMN_NAME';
END;
/